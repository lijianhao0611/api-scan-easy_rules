...
import data_prepare
from data_prepare import load_jsonl_line
...
def merge_all_kit_findings(output_base: str, repo_dir: Path) -> Path:
    ...
    扫描路径: output/{env}/{Kit}/api_scan/api_scan_findings.jsonl
    ...
    for env_dir in output_base.iterdir():
        ...
        for kit_dir in env_dir.iterdir():
            if not kit_dir.is_dir():
                continue
            result_file = kit_dir / "api_scan" / "api_scan_findings.jsonl"
...
def jsonl_to_xlsx_from_records(records: list[dict], xlsx_path: Path) -> int:
    """将记录列表直接转为 XLSX（复用 data_prepare 逻辑）。"""
